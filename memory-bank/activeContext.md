...
3. **删除（管理附件）**：`deleteChatAttachment` 改墓碑（清密文、留行、pending），推送后本地硬删；server 端墓碑清 blob 只留记录且**终态不可复活**（迟到重传被忽略，已冒烟测试 10 断言全过）；`syncChatMessages` 返回 `deletedAttachmentIds`，两端 `stripDeletedAttachmentRefs` 把消息里的悬空引用剥掉（本地重写，不重推）；删除整条消息同样墓碑其附件。`saveChatMessage` 加 `opts.markPending` 让附件删除/描述编辑触发消息重推。删除确认框改为收发双向都弹（文案注明云端+全设备删除不可恢复）。

**追加（2026-08-26 第一批，侧边栏可调高度 + 最近使用 + 安卓折叠/搜索）**：
1. **AI 面板高度拖拽**：`.fn-ai-panel` 与树之间加 `.fn-ai-panel__resize` 分隔条（拖拽设像素高、双击复位自动 45% 上限），`loadAiPanelHeight/saveAiPanelHeight` 持久化，min 72px / max 容器-160px。
2. **最近使用列表**：挂在 `recordEditFocus` 唯一汇合点（开标签/切标签/点内容区/编辑都经过它，头部去重防逐键刷写）；存 30 个 id（`loadRecentNoteIds` 按库命名空间），渲染时过滤文件夹/回收站/已删后取前 10；`.fn-recent-panel` 固定在 AI 面板下、树上方，单击打开+树内定位、双击固定。解锁时重读（切库场景）。
3. **安卓**：NoteTree onSelect 文件夹改为切换折叠（原来无动作、小箭头难点）；抽屉动作行加 ⊞/⊟ 全部展开收起；**全局搜索**——抽屉顶部搜索框，纯内存子串扫描（笔记已全量解密在 state，`noteSearchCorpus` memo 按 notes 变化预计算小写文本，表格 `tableToSearchText` 拍平），结果=标题+两行片段，上限 50，点击打开并关抽屉，锁定清空。

**追加（2026-08-17 后续两轮，Mac 工具复制 + 源码换行）**：① Mac 桌面工具页"复制/全部复制"不可用——Electron 屏蔽 `navigator.clipboard.writeText`，`ToolsPanel` 改用 `copyText`（隐藏 textarea + `execCommand('copy')` 回退，与聊天/表格同款）。② 笔记源码模式自动换行开关（`VaultApp` 工具栏按钮 + `loadSourceWrap` 持久化默认开，`NoteEditor` 用 `Compartment` 动态重配 `EditorView.lineWrapping`）；关闭换行时水平滚动条不可见的修复——外层 `.fn-tab-group__scroll` 加 `--src-nowrap` 变体（外层 overflow-y hidden + flex 撑满），让 CodeMirror 自己接管双轴滚动，水平滚动条钉在编辑器可视底部。

**追加（2026-08-21 第三批，安卓附件下载修复）**：安卓点聊天附件下载无反应的根因——Android System WebView **既不实现 Web Share API 也不处理 blob 锚点下载**，原实现两条路径在设备上全部静默失败。修复：新增依赖 `@capacitor/filesystem@8` + `@capacitor/share@8`（cap sync 已注册，APK 构建验证通过）；新模块 `apps/mobile/src/fileExport.ts` 的 `exportFileNative(fileName, data)`——`Capacitor.isNativePlatform()` 守卫，解密字节 base64 写入 Cache 目录（`fastnote-export/<ts>/<safeName>`，时间戳子目录防同名覆盖），`Share.share({ url })` 弹系统分享面板（FileProvider 自动处理，可存到文件/发给任意应用），用户取消（reject message 含 cancel）视为已处理。`handleChatAttachmentDownload` 改为：原生导出优先 → 浏览器开发环境回退原 Web Share/锚点路径；附件本地缺失时 alert 提示（此前静默 return）。i18n 新键 `chatPanel.attachmentMissing`、`mobileApp.attachmentExportFailed`。该 helper 可复用于后续移动端任何文件导出（AI 导出/工具页导出如需原生化）。

**追加（2026-08-21 第二批，聊天粘贴文件即附件）**：`ChatPanel` composer textarea 加 `onPaste`（`handleComposerPaste`）：`clipboardData.files` 非空时 preventDefault（避免把文件名当文本贴入）并追加进既有 `pendingFiles` 待发附件；剪贴板截图的通用名 `image.png`（或空名）改为 `pasted-YYYYMMDD-HHmmss.<ext>` 时间戳名防止多次粘贴同名。三端共用组件自动生效（安卓已重新 cap sync）。i18n 仅更新 `chatPanel.composerPlaceholderActive` 提示可粘贴。

**追加（2026-08-21 本轮，安卓端笔记 + 表格）**：`apps/mobile` 新增依赖 `@fastnote/editor`/`@fastnote/table`。`MobileApp.tsx` 移植桌面笔记链路的精简版：
1. **数据层**（与桌面字节兼容）：模块级 `newNode`/`buildUpdated`（照抄 VaultApp，hashContent/version/syncStatus 语义一致）；`loadNotesFor`（解锁后**后台** `storage.loadAllNotesDecrypted`，不阻塞解锁；本地库顺手 `purgeDeleted`）；`persistNote`/`saveNoteNow`/`scheduleNotePersist`（500ms 防抖，`pendingNoteSaveRef` 保证 handleLock 时 flush 未落盘的最后一笔编辑）/`updateNoteById`。CRUD：`handleCreateNode`（创建后自动打开并关抽屉）、`handleTrashNote`（子树进回收站）、`handleRestoreNote`（父丢失挂根）、`handleDeleteForever`（登录=墓碑同步，本地=硬删+purge）、`handleEmptyTrash`、`handleMoveNode`（computeTreeMove，按 parentId/sortOrder 变化持久化）。
2. **云同步**：`syncNotes` = `SyncClient.syncAll(notesRef, notesKey, saveNoteNow, storage)`——**必须传 storage**（否则远端删除不落库、下次解锁复活；墓碑推送也依赖它）；`notesLoadedRef` 守卫防止在空列表上跑同步（会漏推本地待同步项）。触发点：本地解锁（restoreChatAfterUnlock **先**同步恢复 session，loadNotesFor 的 purge-vs-sync 判断才正确）、云登录两条路径、抽屉手动 ⟳ 按钮。注意 syncAll 带 storage 会顺跑 attachments/chat/AI 同步（幂等，与 syncChatHistory 少量重复可接受）。
3. **UI**：`view` 增 `'notes'`（顶栏 📝 按钮；无选中笔记时进视图直接开抽屉）。抽屉=新建笔记/表格/文件夹 + ⟳ 同步 + 复用 `NoteTree`（含回收站三件套/重命名/showSyncStatus）；主区=`NoteEditor`（渲染/源码切换 `noteMode`，key=id）或 `TableEditor`（`parseTableDocument`/`serializeTable` 往返，`.fn-mobile__note-pane--table` 双轴滚动）。`handleLock` 清空全部笔记 state。
4. **样式/i18n**：`mobile.css` 新增 notes 区块；**顺手修了该文件历史遗留的错位大括号**（`@media (pointer: coarse)` 提前闭合导致 262-289 行规则游离），coarse 块补 `.fn-tree-node__actions--hover { display:flex }`（触屏常显树行按钮）。i18n 复用 vaultApp/noteTree 键，仅新增 `mobileApp.noNote/openNoteList`。全仓 typecheck + mobile build + cap sync 通过。**已知取舍**：NoteTree 拖拽在触屏无效（HTML5 DnD 限制，移动靠回收站/重建）；无标签页/搜索/附件面板/协作；桌面共享 500ms 单 timer 防抖同款语义。

**追加（2026-08-17 本轮，表格三项增强 + 工具页（桌面+安卓），随 git 提交 minor bump 至 **v0.27.0**，server 无改动）**：
1. **矩形选区 Alt+方向键整体移动**（`TableEditor.handleAltArrowMove`）：原来只有整行/整列/单格三分支，现在任意矩形选区（单格是 1×1 特例）Alt+方向键整体移动一格——相邻的一条单元格带通过**连续相邻 swapCells "冒泡"穿过块**（上移：strip 在 rowStart-1，per 列 swap(r,r+1) 从 rowStart-1 到 rowEnd-1；下移反向；水平同理 per 行），内容/单元格样式/公式引用改写全部复用既有 `swapCells` 链式组合（每次 swap 用自己的输入 doc 算位置，改写可组合）。移动后选区平移 delta，单格且原在编辑态则 rAF 重新聚焦。整列选区垂直移动/整行选区水平移动落入矩形分支时 strip 越界返回 false（无处可换）。已验证 2×2 块上移后 C 列 `=A2+B2` 自动变 `=A1+B1`。
2. **公式编辑时 Excel 风格引用彩色框**：`formula.ts` 新增 `extractFormulaRefs(formula, columnCount, rowCount)`（正则交替按 区间→整列→单格 顺序扫描，支持大小写/$ 锚定/`0x` 无关/开放式 `C1:C` 按整列语义，函数名无冒号无数字不会误配，越界列跳过、行号 clamp）；`TableEditor` 新增 `editingPos` state（cell onFocus 设置 / onEditBlur 清除——编辑内容本就逐键提交进 doc，无需额外文本同步），`formulaRefHighlights` memo 按文档坐标（`docIdxById` Map）解析，第 N 个引用用 `FORMULA_REF_COLORS[N%6]`（蓝红紫绿棕青），td 内联 `boxShadow: inset 0 0 0 2px color`（textarea 透明，与查找高亮同机制），首个匹配引用胜出保持颜色稳定。排序/筛选下引用按文档行号匹配不漂移。
3. **行高应用到所选行**：`utils.setRowHeights(doc, rowIds, height|undefined)`；行高弹层新增「应用到所选行」按钮（无选区禁用），选区覆盖某行任意单元格即视为选中该行（取 displayRows 顺序 rowStart..rowEnd）。i18n `uniformRowHeightApplySelected(+Hint)`，`helpMove` 帮助文案同步矩形选区语义。
4. **工具页（桌面 + 安卓）**：新组件 `packages/ui/src/ToolsPanel.tsx`（`ToolsPanel` + `ToolsSidebar`，`ToolId = 'password' | 'base'`）。**密码生成器**：Chrome 密码管理器风格（默认 16 位，大小写+数字+可选符号每类至少一个），`crypto.getRandomValues` + 拒绝采样（`limit = floor(2^32/max)*max`）避免取模偏差 + 加密随机 Fisher–Yates 洗牌；数量 1–1000、长度 6–128；逐条点击复制/复制全部/导出 txt（复用 shared `downloadBlob`，`passwords-时间戳.txt`）。**进制转换**：BigInt 任意精度，十进制→二进制&十六进制 / 十六进制→二进制&十进制，输入容忍空格/逗号/下划线分隔与 `0x` 前缀，显示 bit 位数，非法输入即时提示。**桌面**：`VaultApp` 的 `AppView` 增 `'tools'`，顶栏 笔记/聊天 后新增「工具」tab，sidebar=ToolsSidebar、main=ToolsPanel。**安卓**：`MobileApp` 的 view 增 `'tools'`，顶栏新增 🛠 按钮（再点回 AI 视图），抽屉复用 ToolsSidebar，标题/抽屉标题按 view 分派。样式统一加在 `apps/web/src/styles.css`（`.fn-tools*`，三端共享）。i18n `vaultApp.navTools` + `toolsPanel.*`。全仓 typecheck + web build 全绿。
