...
- 表格节点：列排序/筛选、增删行列；列/选中区域的统计条（计数、求和、平均值）；类电子表格公式（`=SUM(A1:A3)`、`AVERAGE`、`COUNT`、`MIN`、`MAX`、四则运算与乘方），支持拖拽/整行/整列选中；列数字格式（数字/货币/小数位）、一键插入当前时间；多格复制与智能粘贴分列（生效分隔符可多选自定义：Tab/分号/逗号/空格）；Shift+Enter 单元格内换行，多行单元格通过 Excel 风格引号在复制/粘贴中完整往返；Alt+方向键调整行/列顺序或与相邻单元格交换内容；固定表头/首列、行高列宽调节、单元格加粗/字号/颜色/填充；导出 CSV（明文）/ `.fnxt`（加密），支持导入
...
- 笔记/表格无标题栏（重命名统一走侧边栏），表格功能栏紧凑布局、筛选栏可一键收起，尽量为内容区腾出空间
