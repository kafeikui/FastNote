...
- 侧边栏可折叠的会话树，支持文件夹、重命名、拖拽整理；对话记录与笔记同级加密存储；新建会话/文件夹在当前焦点条目的同层级创建并自动聚焦（可直接 F2 重命名），工具栏一键展开/收起全部文件夹
...
- **Android 移动版（Capacitor）**：`apps/mobile` 是聚焦 AI 助手 + 聊天的移动壳——加密库解锁/创建、加密 AI 会话、流式回复、附件、设置均复用同一套 packages 与库格式；移动端的"转为笔记"改为分享/复制 Markdown；解锁页支持云账号登录（新设备自动采用账号的保险库盐值），登录后即可 1:1 端到端加密聊天并自动同步全部历史聊天记录

**聊天（1:1 端到端加密）**
- 本地永久保存聊天记录（手动删除前不丢失），与是否登录云账号无关
- **登录/解锁后自动同步全部历史聊天记录**（密文 blob 推拉 + 按消息 ID 去重）——新设备登录同一账号也能看到完整历史
...
- 右键消息气泡可**复制完整消息**（含附件文件名列表）；选中部分文字时仍走系统右键复制
...
- Tab 输入制表符：笔记（渲染与源码模式，源码模式下多行选区 Tab 缩进 / Shift+Tab 反缩进）、表格单元格、聊天输入框、AI 输入框中按 Tab 直接输入制表符而不是切换焦点
...
apps/mobile     — Android 移动版（Capacitor WebView，聚焦 AI 助手 + 聊天的壳层）
packages/*      — Web / Electron / Android 共享的业务逻辑（crypto / storage / editor / im / sync / ui / …）
server/         — 自托管中继（Fastify + WebSocket，仅存密文）
```

所有壳层**共享** `packages/*`，只有壳层不同；桌面版额外通过 `window.fastnote` IPC 提供"选择本地数据目录"等原生能力。移动版沿用完全相同的加密库格式（Android WebView 内同样的 IndexedDB 布局），当前提供解锁 + AI 助手 + 端到端加密聊天（含云账号登录与历史聊天同步）；笔记/表格编辑与笔记云同步暂时仍为桌面与 Web 功能。
