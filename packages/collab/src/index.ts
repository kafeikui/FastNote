...
const COLLAB_KDF_SALT_PREFIX = 'fastnote-collab-v1:';
...
/** No confusable glyphs (0/O, 1/I/L) — the code is meant to be read aloud or typed by hand. */
const ROOM_CODE_ALPHABET = 'ABCDEFGHJKMNPQRSTUVWXYZ23456789';

/**
 * Generates a random room code (e.g. "K7GM-PW3Q") for a new collaboration room. Without a random
 * per-room component, two unrelated documents whose owners happened to pick the same password
 * would land in the same relay room and cross-sync; the initiator generates this once and shares
 * it out of band together with the password.
 */
...
/** Tolerates lowercase / missing dash / stray whitespace when a joiner types the code by hand. */
export function normalizeCollabRoomCode(code: string): string {
  return code.toUpperCase().replace(/[^A-Z0-9]/g, '');
}

/**
 * Derives the room identity and payload key from the shared collaboration password and room
 * code. Both derivations are deterministic so every participant lands in the same room with the
 * same key, while the server-visible roomId cannot be reversed into the key (independent HKDF
 * infos). The room code acts as the KDF salt: it disambiguates rooms that share a password and
 * defeats any precomputation against a fixed salt.
 */
export async function deriveCollabRoom(password: string, roomCode: string): Promise<CollabRoom> {
  ...
  const bits = await crypto.subtle.deriveBits(
    {
      name: 'PBKDF2',
      salt: enc.encode(COLLAB_KDF_SALT_PREFIX + normalizeCollabRoomCode(roomCode)),
      iterations: 600_000,
      hash: 'SHA-256',
    },
    keyMaterial,
...
interface CollabMessage {
  ...
  patches?: string;
  /** Document title: carried by `state` (bootstrap) and `title` (rename, whole-value LWW). */
  title?: string;
}
...
export interface CollabSessionOptions {
  ...
  password: string;
  /** The room code generated by the initiator (see generateCollabRoomCode) and shared with the password. */
  roomCode: string;
  ...
  /** Returns the current local document title (synced as a whole value, last-writer-wins). */
  getTitle?: () => string;
  /** Applies a remotely changed title; must not feed back into `updateLocalTitle`. */
  applyRemoteTitle?: (title: string) => void;
  /**
...
export class CollabSession {
  ...
  /** Last title exchanged with the room; suppresses echoes and no-op broadcasts. */
  private titleShadow: string;
  ...
  private constructor(
    ...
    this.titleShadow = opts.getTitle?.() ?? '';
  }

  static async create(opts: CollabSessionOptions): Promise<CollabSession> {
    const room = await deriveCollabRoom(opts.password, opts.roomCode);
  ...
  /** Feed every local rename here; titles sync as whole values (last-writer-wins). */
  updateLocalTitle(_title: string): void {
    if (this.disposed) return;
    if (this.titleFlushTimer) clearTimeout(this.titleFlushTimer);
    this.titleFlushTimer = setTimeout(() => this.flushLocalTitle(), LOCAL_FLUSH_DEBOUNCE_MS);
  }
  ...
  private async handleFrame(raw: string): Promise<void> {
    ...
    if (msg.kind === 'state' && typeof msg.text === 'string') {
      this.handleState(msg.text, msg.title);
      return;
    }

    if (msg.kind === 'title' && typeof msg.title === 'string') {
      this.applyTitle(msg.title);
      return;
    }
  ...
  private applyTitle(title: string): void {
    this.titleShadow = title;
    if (this.opts.applyRemoteTitle && title !== (this.opts.getTitle?.() ?? '')) {
      this.opts.applyRemoteTitle(title);
    }
  }

  private handleState(remoteText: string, remoteTitle?: string): void {
    if (this.awaitingState) {
      ...
      if (typeof remoteTitle === 'string') this.applyTitle(remoteTitle);
      return;
    }
    if (typeof remoteTitle === 'string' && remoteTitle !== this.titleShadow) this.applyTitle(remoteTitle);
  ...
  private flushLocalTitle(): void {
    if (this.disposed || !this.opts.getTitle) return;
    const title = this.opts.getTitle();
    if (title === this.titleShadow) return;
    this.titleShadow = title;
    void this.sendMessage({ kind: 'title', client: this.clientId, seq: ++this.seq, title });
  }
}
